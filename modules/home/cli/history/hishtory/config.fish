...
function _hishtory_post_exec --on-event fish_preexec
    ...
    hishtory presaveHistoryEntry fish "$_hishtory_command" $_hishtory_start_time &
    disown 2>&1 >/dev/null # Background Run
...
if test (hishtory config-get enable-control-r) = true
    if test $fish_key_bindings = fish_vi_key_bindings
        bind --mode insert \cr __hishtory_on_control_r
    else
