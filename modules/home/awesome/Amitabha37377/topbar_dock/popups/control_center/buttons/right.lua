...
local create_button = function(container_widget, text, command1, command2, bg_click)
  ...
  local button_text = wibox.widget({
    {
      {
        markup = '<span color="' .. color.blueish_white .. '" font="Ubuntu Nerd Font bold 11">' .. text .. "</span>",
        font = "Ubuntu Nerd Font Bold 14",
        widget = wibox.widget.textbox,
        fg = color.white,
      },
  ...
  button_image:connect_signal("button::press", function()
    ...
    if onstatus then
      ...
      status:set_markup_silently('<span color="' .. color.white .. '" font="Ubuntu Nerd Font 11">' .. "On" .. "</span>")
      awful.spawn.with_shell(command1)
    else
      container_widget:set_bg(color.grey)
      status:set_markup_silently(
...
local right_buttons = {
  ...
  nightmode = create_button(
    container.dark,
    "Night Mode",
    "redshift -l 0:0 -t 4500:4500 -r",
